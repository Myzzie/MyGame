...
[System.Serializable]
public class OreAmountManager : MonoBehaviour {
	

	static OreAmountManager baseOreAmountManager;
	...
	void Awake()
	{
		...
		baseOreAmountManager = this;
		baseOreAmountManager.instance.SetActive(false);
	}
	...
	void Update()
	{
		...
		else if (count == 5) 
		{
			...
			{
				if (Materials.materials.mineLevel >= 60)
				{
					button.GetComponent<Button>().interactable = true;
				}
				else button.GetComponent<Button>().interactable = false;
